...
class V016ImportTest {
    ...
    private fun automatic(a:ShapeArchive,old:List<CatalogItem> = emptyList(),mode:String?=null):ImportPlan {
        val mapping=a.layers.map{ImportPlanner.propose(it)}
        return ImportPlanner.plan(a,mapping,"automatic-test","",mode?:ImportPlanner.automaticMode(a,mapping),true,false,5.0,old,"test")
    }
    @Test fun zipWithoutCollectorOrDisplayCodeCreatesStableCollectorAndCodesFromKeys(){
        ...
        assertEquals("ORDERED",ImportPlanner.automaticMode(a,a.layers.map{ImportPlanner.propose(it)}));val first=automatic(a,mode="ISOLATED");assertEquals(1,first.items.count{it.kind=="collector"});assertEquals(3,first.items.count{it.kind=="point"});assertEquals(0,first.items.count{it.kind=="segment"})
        ...
        val repeated=automatic(a,first.items,"ISOLATED");assertEquals(0,repeated.inserted);assertEquals(0,repeated.updated)
