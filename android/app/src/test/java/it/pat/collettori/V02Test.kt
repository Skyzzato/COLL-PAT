...
class V02Test {
    ...
    @Test fun deletedSourceRecordCannotBeReimported(){val l=layer();val a=plan(l);denied{plan(l,old=a.items.map{if(it.kind=="point")it.copy(body=JSONObject(it.body).put("deleted",true).toString())else it})}}
    ...
    @Test fun publicationOrderAndBuildAreCompatible(){listOf("0.1","0.11","0.12","0.13","0.14","0.15","0.16").forEach{assertTrue(compareVersions("0.2",it)>0)};assertEquals("0.2",BuildConfig.VERSION_NAME);assertTrue(BuildConfig.VERSION_CODE>16)}
