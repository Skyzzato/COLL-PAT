...
class V013Test {
    ...
    @Test(expected=IllegalArgumentException::class) fun orderRequiresConfirmation(){val a=read("points");ImportPlanner.plan(a,a.layers.map{ImportPlanner.propose(it)},"source",collector.getString("id"),"ORDERED",false,false,0.0,listOf(CatalogItem("test",collector.getString("id"),"collector",collector.toString())),"test")}
