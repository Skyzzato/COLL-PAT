...
@Composable fun InspectionEditor(repo:Repository,visit:Visit,point:JSONObject?,collectorLabel:String,busy:Boolean,message:String,onMessage:(String)->Unit,onBack:()->Unit,captureFactory:()->InspectionCapture={LocationCapture(repo.context)}){
    ...
    feedback?.let{saved->
        ...
        AlertDialog(onDismissRequest={leave()},title={Text(saved.title(visit,!repo.authenticated()))},text={Column{Text(saved.detail(visit,!repo.authenticated()));if(photoPending)Text("Fotografie: caricamento ancora in coda");Text("Ritorno alla pagina precedente tra 2 secondi")}},confirmButton={TextButton(onClick={leave()}){Text("Torna ora")}})
    }
