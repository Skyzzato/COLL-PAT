...
@Composable fun InspectionEditor(repo:Repository,visit:Visit,point:JSONObject?,collectorLabel:String,busy:Boolean,message:String,onMessage:(String)->Unit,onBack:()->Unit,onCapture:()->Unit,capturing:Boolean,onCancelCapture:()->Unit){
    ...
    fun leave(){if(committing)return;committing=true;scope.launch{latestWrite?.join();try{if(draft)repo.saveDraft(visit.id,JSONObject(bodyText));onBack()}catch(e:Exception){onMessage(e.message?:"Salvataggio non riuscito")}finally{committing=false}}}
    BackHandler{leave()}
    fun commit(status:String?){
        if(committing)return;committing=true;val submitted=JSONObject(bodyText)
        scope.launch{try{latestWrite?.join();if(status==null){repo.saveDraft(visit.id,submitted);onMessage("Bozza salvata · solo sul telefono")}
            else{repo.complete(visit.id,submitted,status);onMessage("Registrazione accodata · in attesa della ricevuta server")}
        }catch(e:Exception){onMessage(e.message?:"Registrazione non riuscita")}finally{committing=false}}
    }
    Column(Modifier.fillMaxSize().statusBarsPadding().navigationBarsPadding().padding(16.dp).verticalScroll(rememberScrollState())){
        ...
        Text("${operational(visit.operational)} · ${syncLabel(visit.sync)}",style=MaterialTheme.typography.labelLarge)
        ...
        val persisted=JSONObject(visit.body);val events=persisted.getJSONArray("events").objects();val last=lastEvidence(persisted)
        ...
        if(persisted.optBoolean("evidence_rectified"))Text("Evidenza rettificata: il riepilogo originale è conservato. ${if(last==null)"Nessuna evidenza attiva." else "Consultare l'audit."}",color=MaterialTheme.colorScheme.error)
        ...
        if(draft){
            ...
            Button(enabled=!saving,onClick={commit("COMPLETO")},modifier=Modifier.fillMaxWidth().heightIn(min=56.dp)){ActionIcon(R.drawable.ic_check);Text("Registra Ispezione")}
    ...
    if(cancelVisit||cancelEvent!=null)CancelDialog(if(cancelVisit)"ispezione" else "rilevazione",{cancelVisit=false;cancelEvent=null}){reason->val event=if(cancelVisit)null else cancelEvent;scope.launch{try{latestWrite?.join();repo.cancel(visit.id,event,reason);onMessage("Annullamento salvato con audit")}catch(e:Exception){onMessage(e.message?:"Annullamento non riuscito")}};cancelVisit=false;cancelEvent=null}
}
val fieldLabels=mapOf("cover" to "Botola e telaio","deposits" to "Depositi e materiali estranei","flow" to "Deflusso","walls" to "Pareti e canalette","damage" to "Danni, infiltrazioni e radici","closure" to "Richiusura","restored" to "Ripristino area","surface" to "Condizioni del manto","subsidence" to "Avvallamenti / cedimenti")
