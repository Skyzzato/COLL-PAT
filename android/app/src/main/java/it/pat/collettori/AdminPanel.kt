...
@Composable fun AccountPanel(repo:Repository,onChange:()->Unit){
    ...
    Column{Text(repo.store.get()?.optString("username")?:"Nessun account")
        ...
        if(mayManageCatalog(repo.store.get())){
            Text("Simula abilitazione utente",style=MaterialTheme.typography.titleMedium)
            Text("Anteprima dei comandi, senza nuove scritture operative.")
            Row{TextButton(onClick={repo.simulate("viewer")}){Text("Visualizzatore")};TextButton(onClick={repo.simulate("inspector")}){Text("Operatore")}}
            if(repo.simulatedRole()!=null)TextButton(onClick={repo.simulate(null)}){Text("Termina simulazione")}
            if(repo.canManageUsers())OutlinedButton(onClick={users=true}){Text("Gestione utenti")}
        }
        OutlinedButton(onClick={repo.simulation.value=null;repo.store.clear();onChange()}){Text("Logout")}}
    if(users&&repo.canManageUsers())UserManagementDialog(repo){users=false}
}
...
@Composable fun AdminPanel(repo:Repository,catalog:List<CatalogItem>,visitCount:Int,onMessage:(String)->Unit){
    ...
    val appearanceRaw by repo.dao.settingFlow(repo.owner(),"field-settings-v014").collectAsState(null)
    val appearanceSettings=remember(appearanceRaw){appearanceRaw?.let{FieldSettings.parse(JSONObject(it))}?:FieldSettings()}
    ...
    form?.let{c->CollectorForm(c,catalog,{form=null},appearanceSettings){next,points->task{repo.saveCatalog(listOf(CatalogItem(repo.owner(),next.getString("id"),"collector",next.toString()))+points.map{CatalogItem(repo.owner(),it.getString("id"),"point",it.toString())});form=null;onMessage("Collettore e pozzetti salvati localmente · accodati al server")}}}
...
@Composable fun CollectorForm(initial:JSONObject,catalog:List<CatalogItem>,onDismiss:()->Unit,settings:FieldSettings=FieldSettings(),onSave:(JSONObject,List<JSONObject>)->Unit){
    ...
    Dialog(onDismissRequest=onDismiss,properties=DialogProperties(usePlatformDefaultWidth=false)){Surface(Modifier.fillMaxSize().statusBarsPadding().navigationBarsPadding()){Column(Modifier.verticalScroll(rememberScrollState()).padding(20.dp)){
        ...
        CollapsibleItems("Pozzetti",points,{it.getString("id")}){p->TextButton(onClick={editingPoint=JSONObject(p.toString())}){Text("#${p.getString("code")} · Modifica dati")}}
        ...
        }catch(e:Exception){if(e is kotlinx.coroutines.CancellationException)throw e;error=friendlyError(e)}}){Text("Salva collettore e pozzetti")};TextButton(onClick=onDismiss){Text("Indietro")}
    }}}
    editingPoint?.let{p->PointForm(p,initial,points,catalog.filter{it.kind=="segment"}.map{JSONObject(it.body)},{editingPoint=null},settings){next->points=points.filter{it.getString("id")!=next.getString("id")}+next;changed=changed+next.getString("id");editingPoint=null}}
}
