        ...
        if(plan==null){
            ...
            archive?.let{a->
                ...
                a.layers.forEach{layer->
                    ...
                    if(!report.valid)Text("Non è stato trovato un identificativo univoco. Apri Opzioni avanzate per scegliere il campo corretto o correggi il file sorgente. Nessun aggiornamento ambiguo sarà importato.")
            ...
            Button(enabled=!busy,onClick={task{repo.saveCatalog(p.items,p.provenance);repo.dao.setting(Setting(repo.owner(),"import-options:$source",JSONObject().apply{archive!!.layers.forEach{layer->put(layer.name,JSONObject().put("encoding",layer.encodingChoice).put("key",mappings.first{it.layer==layer.name}.fields["key"]))}}.toString()));message("Importazione salvata. Sincronizzazione automatica in attesa di conferma dal server.");dismiss()}}){Text("Conferma importazione")}
