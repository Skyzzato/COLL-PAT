        ...
        if(plan==null){
            ...
            archive?.let{a->
                ...
                a.layers.forEach{layer->
                    ...
                    keys.forEach{k->Choice(mapOf("key" to "Chiave sorgente stabile","code" to "Codice (testo)","description" to "Descrizione","collector" to "Codice collettore","asset_type" to "Tipo manufatto (MANHOLE = pozzetto)","under_asphalt" to "Sotto asfalto (sì/no)","sequence" to "Sequenza ordinale","chainage" to "Progressiva GIS (metri)","branch" to "Ramo","previous" to "Chiave precedente","next" to "Chiave successivo","from" to "Chiave estremo iniziale","to" to "Chiave estremo finale")[k]!!,map.fields[k]?:"",listOf("" to "Non presente")+layer.fields.map{it to it}){field->mappings=mappings.map{if(it.layer==layer.name)it.copy(fields=it.fields+(k to field))else it}}}
                }
