...
class Api(private val store:SessionStore,private val client:OkHttpClient=OkHttpClient.Builder().connectTimeout(15,java.util.concurrent.TimeUnit.SECONDS).readTimeout(45,java.util.concurrent.TimeUnit.SECONDS).build()){
    ...
    suspend fun raw(base:String,path:String,key:String,token:String?=null,body:JSONObject?=null,bytes:ByteArray?=null):ByteArray=withContext(Dispatchers.IO){
        ...
        suspendCancellableCoroutine{continuation->
            ...
            call.enqueue(object:Callback{
                ...
                override fun onResponse(call:Call,response:Response){response.use{r->try{
                    ...
                    if(!r.isSuccessful){val error=runCatching{JSONObject(String(data))}.getOrNull();throw ApiError(r.code,error?.optString("message",error.optString("error_description","Richiesta rifiutata"))?.take(500)?:"Richiesta rifiutata")}
    ...
    suspend fun request(path:String,body:JSONObject?=null,expectedOwner:String?=null,bytes:ByteArray?=null):ByteArray{
        ...
        try{return raw(original.getString("base"),path,original.getString("public_key"),original.getString("access_token"),body,bytes)}catch(e:ApiError){
            ...
            return refreshLock.withLock{
                ...
                raw(next.getString("base"),path,next.getString("public_key"),next.getString("access_token"),body,bytes)
            }
        }
    }
}
