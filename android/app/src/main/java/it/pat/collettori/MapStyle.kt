...
fun localStyle(pack:JSONObject,points:List<JSONObject>,selected:String?,position:JSONObject?,settings:FieldSettings=FieldSettings()):String{
    ...
    val layers=JSONArray("""[
        ...
        {"id":"manholes","type":"symbol","source":"points","layout":{"icon-allow-overlap":true,"icon-ignore-placement":true},"paint":{"icon-color":["get","status_color"],"icon-halo-color":"#ffffff","icon-halo-width":1}},
    ...
    layers.objects().first{it.getString("id")=="manholes"}.getJSONObject("layout")
        .put("icon-image",ManholeSymbol.fromId(settings.symbol).imageId).put("icon-size",settings.iconSize/ManholeSymbol.RADIUS)
